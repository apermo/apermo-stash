...
class PermissionsTest extends TestCase {
	...
	/**
	 * Verifies require_read_links denies and uses a read-oriented message.
	 *
	 * @return void
	 */
	public function test_require_read_links_denies_with_read_message(): void {
		Functions\when( 'current_user_can' )->justReturn( false );

		$result = Permissions::require_read_links();
	...
	/**
	 * Verifies require_read_links allows users with edit_posts.
	 *
	 * @return void
	 */
	public function test_require_read_links_allows_when_cap_present(): void {
		Functions\when( 'current_user_can' )->justReturn( true );

		self::assertTrue( Permissions::require_read_links() );
	}

	/**
	 * Verifies can_edit_link delegates to current_user_can( 'edit_post', id ).
	 *
	 * @return void
	 */
	public function test_can_edit_link_passes_id_to_cap_check(): void {
	...
	/**
	 * Verifies can_delete_link delegates to current_user_can( 'delete_post', id ).
	 *
	 * @return void
	 */
	public function test_can_delete_link_passes_id_to_cap_check(): void {
		Functions\expect( 'current_user_can' )
			->once()
			->with( 'delete_post', 42 )
			->andReturn( false );
	...
	/**
	 * Verifies can_read_link returns 404 for an unknown post.
	 *
	 * @return void
	 */
	public function test_can_read_link_404_when_post_missing(): void {
		Functions\when( 'get_post' )->justReturn( null );

		$request         = new WP_REST_Request();
		$request->params = [ 'id' => 999 ];

		$result = Permissions::can_read_link( $request );
		self::assertInstanceOf( WP_Error::class, $result );
		self::assertSame( 'apermo_stash_not_found', $result->code );
	}

	/**
	 * Verifies can_read_link allows anyone when the link is public.
	 *
	 * @return void
	 */
	public function test_can_read_link_allows_publish(): void {
		$post              = new WP_Post();
		...
		self::assertTrue( Permissions::can_read_link( $request ) );
	}

	/**
	 * Verifies can_read_link allows the owner on a private link.
	 *
	 * @return void
	 */
	public function test_can_read_link_allows_owner_on_private(): void {
		$post              = new WP_Post();
		$post->post_type   = LinkPostType::POST_TYPE;
	...
	/**
	 * Verifies can_read_link allows users with edit_others_posts.
	 *
	 * @return void
	 */
	public function test_can_read_link_allows_admin(): void {
		$post              = new WP_Post();
		$post->post_type   = LinkPostType::POST_TYPE;
		$post->post_status = 'private';
		$post->post_author = 11;
	...
	/**
	 * Verifies can_read_link denies a non-owner without admin caps,
	 * returning 404 (indistinguishable from "post does not exist") rather
	 * than 403 to prevent ID-enumeration of private links.
	 *
	 * @return void
	 */
	public function test_can_read_link_denies_non_owner_with_404(): void {
		$post              = new WP_Post();
		$post->post_type   = LinkPostType::POST_TYPE;
		$post->post_status = 'private';
		$post->post_author = 11;
